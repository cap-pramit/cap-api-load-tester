...
describe('Listings page', () => {
  ...
  it('Should be able to see contents of application listings table', async () => {
    initializeApp();

    expect(
      await screen.findByText(/vulcan applications/i, {}, null, {
        timeout: 10000,
      }),
    ).toBeInTheDocument();
    expect(
      await screen.findByText(/type/i, {}, null, {
        timeout: 10000,
      }),
    ).toBeInTheDocument();
    expect(
      await screen.findByText(/language/i, {}, null, {
        timeout: 10000,
      }),
    ).toBeInTheDocument();
    expect(
      (await screen.findAllByText(/react/i, {}, null, {
        timeout: 10000,
      }))[0],
    ).toBeInTheDocument();
    expect(
      await screen.findByText(/nikhil_000999/i, {}, null, {
        timeout: 10000,
      }),
    ).toBeInTheDocument();
  });
