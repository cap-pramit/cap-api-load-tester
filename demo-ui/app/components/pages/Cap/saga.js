...
function* getOrgUsersData({successCallback}) {
  try {
    const res = yield call(Api.getOrgUsersData);
    if (res.success) yield put({ type: actionTypes.GET_ORG_USERS_DATA_SUCCESS, data: [...res?.result?.orgUsers, ...res?.result?.capUsers] });
    if (successCallback) successCallback();
  } catch (error) {
    yield put({ type: actionTypes.GET_ORG_USERS_DATA_FAILURE, error });
  }
}
...
function* watchForGetOrgUsersData() {
  yield takeLatest(actionTypes.GET_ORG_USERS_DATA_REQUEST, getOrgUsersData);
}
