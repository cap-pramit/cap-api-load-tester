...
const makeSelectOrgUsers = () =>
  createSelector(selectCapDomain, (substate = fromJS({})) => {
    const capAndOrgUsers = substate.get('orgUsers')?.toJS();

    const allUsersMap = new Map();
    capAndOrgUsers?.forEach(data => {
      allUsersMap.set(data?.userId, `${data?.firstName} ${data?.lastName}`);
    });
    return allUsersMap;
  });
